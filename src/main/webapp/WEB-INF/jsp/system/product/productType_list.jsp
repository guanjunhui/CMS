...
$(function(){
	...
	$("#ss").click(function(){
		...
		$.ajax({
			type: "POST",
			url:adminPath+"productType/getlistTree.do",
...
function edit(data){
	window.location.href=adminPath+"productType/toEdit.do?id="+data; 
}
function setType_status(data,sta){//
	 $.ajax({
		url:adminPath+"productType/updataStatus.do",
		type:"post",
...
function batchDel(param){
	hideConfirm();
	$.ajax({
		url:adminPath+"productType/delete.do",
		type:"get",
		data:param,
		...
		html+='<div class="dd_tit cf">';
			...
			html+='<div class="sele06 sanJi">';
				...
				html+='<ul class="guanli_con cf">';
					...
					if(status=='1'){
					    html+='<li><a href="javascript:;" onclick="setType_status(\''+obj.id+'\',0)">白色</a></li>';
					}else{
						html+='<li><a href="javascript:;" onclick="setType_status(\''+obj.id+'\',1)">黑色</a></li>';
					}
...
function typeSort(param){
	...
	if(zz.test(sort)){
		$.ajax({
			type: "GET",
			url:adminPath+"productType/updateSort.do",
		...
		<div class="cms_c_list biaoge_con chanpin_con zhandian_con cf">
			<div class="h3 cf">
				<div class="h3_left cf">
					<a href="<%=adminPath%>productType/toAdd.do">+添加分类</a>
