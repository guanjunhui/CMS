		...
		$(document).on('mouseleave','.yulan',function(){
		    ...
		    $("#type_name").blur(function(){
				$.ajax({
					...
					data:{type_name:$("#type_name").val(),id:$("#id").val()},
...
function sub(type){
	...
	$("#Form").submit();
	    /* var formData = new FormData($("#Form")[0]);
		 $.ajax({
			type: "POST",
			url:adminPath+"productType/edit.do?columId="+columId+"&columType="+columType+"",
			data:formData,
			dataType:'json',
			cache: false,
	        processData: false,
			...
			success: function(result){
				 if(result.success){
					 if(type=='1'){//继续添加
						 ...
						 location.href=adminPath+'productType/toAdd.do';
					 }else{
						 window.top.mesageTip("success","操作成功");
...
<body>
	...
	<div class="cms_con cf">
		...
		<div class="cms_c_list cf">
			...
			<form name="productForm" action="<%=adminPath%>productType/edit.do" id="Form" method="post"
							...
							<div class="dd_con">
								<div class="show_list" style="margin-top: 0px;">
									<table class="ev">
										...
										<tbody id="typeBody">
											<c:if test="${!empty productType.pname }">
												<tr>
													<td width='50%'>${productType.pname }</td>
													<td><a href='javascript:;'
														onclick='removeBody(this,"#typediv :checked")'>移除</a><input
														type='hidden' value="${product.pid }" /></td>
												</tr>
											</c:if>
										</tbody>
									</table>
							...
							<div class="dd_con">
								<div class="show_list" style="margin-top: 0px;">
									<table class="ev">
										...
										<tbody id="columnBody">
											<c:forEach items="${productType.columConfigList }" var="v">
												<tr>
							...
							<dd>
								<div class="dd_con">
									<input type="text" id="type_name" name="type_name"
										value="${productType.type_name }" required><span
							...
							<dd class="zp_dd">
								<c:if test="${productType.type_wburl != '' && productType.type_wburl != null}">
									...
									<div class="dd_con" style="display: block;">
										<input type="text" style="width: 300px;" id="type_wburl" name="type_wburl" value="${productType.type_wburl }" class="url">
									</div>
								</c:if>
								<c:if test="${productType.type_wburl == '' || productType.type_wburl == null}">
												...
												<div class="yulan"
													...
													<div class="pro_img"
														...
														<div class="pro_img_big">
															<img id="preview" alt=""
																src="<%=imgPath%>${productType.imgurl}" width="150px"
																height="180px">
														</div>
													</div>
												</div>
							...
							<div class="dd_con show_hide">
								<c:if test="${productType.type_status==1 }">
									...
									<label for="hide"><input type="radio" id="hide"
										name="type_status" value="0" /><span>隐藏</span></label>
								</c:if>

								<c:if test="${productType.type_status==0 }">
									<label for="show" class="active"><input type="radio"
										id="show" name="type_status" value="1" /><span>显示</span></label>
									<label for="hide"><input type="radio" checked="checked"
										id="hide" name="type_status" value="0" /><span>隐藏</span></label>
								</c:if>
							</div>
						</dd>
					</dl>
									...
									<table id="table_report"
										...
										<tr>
											<td style="z-index: 1;"><script id="editor"
													type="text/plain"
													style="width: 90.2%; height: 240px; z-index: 1;">${productType.type_txt }</script>
											</td>
										</tr>
