...
<script type="text/javascript">
	...
	function detailById(id){
		window.location.href=adminPath+"registration/detailById.do?id="+id;
	}
	//通过id删除数据
	...
	//更新申保数据也就是修改banner数据
	function updateRecommend(id,recommend){
		/* if(id==0){
			return false;
		}
		...
		$.ajax({
            url:"${basePath}/product/registration/updataRecommend",
            type:"post",
            data:{
            	"id":id,
            	"recommend":recommend
            ...
            success:function(data){
           	 	  if (data.code == 200) {
           			window.location.href="${basePath}/manage/registration/list.do";
		          }else{
		        	  alert("用户名或密码不正确");
		          }
            }
				...
				<div class="table cf">
					...
					<dl class="list_bg col_07 cf" id="did">
						<dt class="cf">
							...
							<div class="sele07">国家</div>
							<div class="sele07 other_width">电话</div>
							<div class="sele07">邮编</div>
									...
									<div class="dd_tit cf">
										<div class="tit7">
											<div class="tit7_con">
												...
												<c:if test="${map.recommend==1 }"><i style="color:green;">[已处理]</i>${map.fullName}</c:if>
												<c:if test="${map.recommend!=1 }">${map.fullName}</c:if>
											</div>
										</div>
										<div class="sele07 sanJi">
											<a href="javascript:;">管理<span class="sanjiao"></span></a>
											<ul class="guanli_con cf">
												<c:if test="${map.recommend==1 }"><li><a onclick="updateRecommend('${map.id }',0)">已处理</a></li></c:if>
												<c:if test="${map.recommend!=1 }"><li><a onclick="updateRecommend('${map.id }',1)">未处理</a></li></c:if>
