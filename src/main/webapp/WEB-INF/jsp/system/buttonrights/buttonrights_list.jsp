...
<body class="no-skin">
									...
									<td style="width:50px;">角色组:</td>
										...
										<c:forEach items="${roleList}" var="role" varStatus="vs">
											<td style="width:100px;" class="center" <c:choose><c:when test="${pd.ROLE_ID == role.ROLE_ID}">bgcolor="#FFC926" onMouseOut="javascript:this.bgColor='#FFC926';"</c:when><c:otherwise>bgcolor="#E5E5E5" onMouseOut="javascript:this.bgColor='#E5E5E5';"</c:otherwise></c:choose>  onMouseMove="javascript:this.bgColor='#FFC926';" >
												<a href="<%=adminPath%>buttonrights/list.do?ROLE_ID=${role.ROLE_ID }" style="text-decoration:none; display:block;"><i class="menu-icon fa fa-users"></i><font color="#666666">${role.ROLE_NAME }</font></a>
											</td>
							...
							<table style="width:100%;">
								<tr>
									<td style="vertical-align:top;">
										<c:if test="${QX.chas == 1 }">
										<a class="btn btn-mini btn-success" onclick="window.location.href='<%=adminPath%>buttonrights/list.do?type=2';">切换视图</a>
										</c:if>
									</td>
								</tr>
							</table>
	...
	<script type="text/javascript">
		...
		function upRb(ROLE_ID,FHBUTTON_ID){
			$.ajax({
				type: "POST",
				url: "<%=adminPath%>buttonrights/upRb.do?ROLE_ID="+ROLE_ID+"&BUTTON_ID="+FHBUTTON_ID+"&guid="+new Date().getTime(),
		    	data: encodeURI(""),
				dataType:'json',
