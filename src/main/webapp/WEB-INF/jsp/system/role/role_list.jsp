...
<body class="no-skin">
									...
									<td style="width:69px;"><a href="javascript:addRole(0);" class="btn btn-sm btn-success">新增组</a></td>
										...
										<c:forEach items="${roleList}" var="role" varStatus="vs">
											<td style="width:100px;" class="center" <c:choose><c:when test="${pd.ROLE_ID == role.ROLE_ID}">bgcolor="#FFC926" onMouseOut="javascript:this.bgColor='#FFC926';"</c:when><c:otherwise>bgcolor="#E5E5E5" onMouseOut="javascript:this.bgColor='#E5E5E5';"</c:otherwise></c:choose>  onMouseMove="javascript:this.bgColor='#FFC926';" >
												<a href="<%=adminPath%>role.do?ROLE_ID=${role.ROLE_ID }" style="text-decoration:none; display:block;"><i class="menu-icon fa fa-users"></i><font color="#666666">${role.ROLE_NAME }</font></a>
											</td>
	...
	<script type="text/javascript">
		...
		function addRole(pid){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=adminPath%>role/toAdd.do?parent_id='+pid;
			 diag.Width = 222;
			 diag.Height = 100;
		...
		function editRole(ROLE_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=adminPath%>role/toEdit.do?ROLE_ID='+ROLE_ID;
			 diag.Width = 222;
			 diag.Height = 100;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delRole(ROLE_ID,msg,ROLE_NAME){
			bootbox.confirm("确定要删除["+ROLE_NAME+"]吗?", function(result) {
				if(result) {
					var url = "<%=adminPath%>role/delete.do?ROLE_ID="+ROLE_ID+"&guid="+new Date().getTime();
					top.jzts();
		...
		function editRights(ROLE_ID){
			 ...
			 diag.Drag = true;
			 diag.Title = "菜单权限";
			 diag.URL = '<%=adminPath%>role/menuqx.do?ROLE_ID='+ROLE_ID;
			 diag.Width = 320;
			 diag.Height = 450;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		function roleButton(ROLE_ID,msg){
			...
			if(msg == 'add_qx'){
			 ...
			 diag.Drag = true;
			 diag.Title = Title;
			 diag.URL = '<%=adminPath%>role/b4Button.do?ROLE_ID='+ROLE_ID+'&msg='+msg;
			 diag.Width = 330;
			 diag.Height = 450;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
