...
function appendTypediv(list){
	 ...
	 $.each(list,function(index,obj){
		 html+='<li>';
		 	html+='<p><em class="show_btn"></em><label for="cc_'+obj.id+'"><input type="radio" att="'+obj.typeName+'" name="pId" value="'+obj.id+'" id="cc_'+obj.id+'" /><span>'+obj.typeName+'</span><i></i></label></p>';
		 	if(obj.childList!=null && obj.childList.length>0){
...
function sub(type){
	...
	$.ajax({
		type: "POST",
		url:adminPath+"contentType/save.do?columId="+columId+"&columType="+columType+"",
		data:formData,
		dataType:'json',
		cache: false,
        processData: false,
		...
		success: function(result){
			 if(result.success){
				 if(type=='1'){//继续添加
					 ...
					 location.href=adminPath+'contentType/toAdd.do';
				 }else{
					 window.top.mesageTip("success","操作成功");
...
<body>
	...
	<div class="cms_con cf">
		...
		<div class="cms_c_list cf">
			...
			<form name="contentForm" id="Form" method="post"
				enctype="multipart/form-data" action="<%=adminPath%>contentType/save.do">
				...
				<div class="add_btn_con wrap cf">
					...
					<div class="zhaopin zp_con01 cf">
						<dl class="cf zp_dl">
							...
							<dd>
								<div class="dd_con">
									<input type="text" id="type_name" name="typeName" required><span
						...
						<dd>
							<div class="dd_con show_hide">
								...
								<label for="show" class="active"><input
									...
									type="radio" id="hide" name="typeStatus" value="0" /><span>隐藏</span></label>
						...
						<dd>
							<div class="dd_con">
								<ul>
									<li>
										<h6>
											<textarea class="textarea_numb" name="typeSummary"></textarea>
											<p>
					...
					<dl class="cf">
						...
						<dd>
							<div class="dd_con">
								<input type="text" name="typePosition"/>
							</div>
						</dd>
					</dl>
					<dl class="cf">
					...
					<dl class="cf">
							<dt>SEO描述</dt>
							<dd><div class="dd_con"><input type="text" name="typeDescription" /></div></dd>
						</dl>
				</div>
				<div class="all_btn cf">
					<span id="result"></span> <input type="button" class="submit_btn"
						...
						href="<%=adminPath%>columcontent_colum/managecontentType.do?ID=${columId}&topColumId=${topColumId}"
