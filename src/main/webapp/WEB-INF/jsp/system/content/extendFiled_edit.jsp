...
function save(type){
	...
	var formData=$("#Form").jsonObject();
		$.ajax({
			type: "POST",
			url:adminPath+"contentExtendFiledController/update.do",
			data:formData,
			success: function(result){
				 if(result.success){
					 if(type=='1'){//继续添加
						 window.top.mesageTip("success","操作成功");
						 location.href=adminPath+'contentExtendFiledController/toAdd.do';
					 }else{
						 window.top.mesageTip("success","操作成功");
						 location.href=adminPath+'contentExtendFiledController/list.do';
					 }
				 }else{
	...
	<div class="cms_con cf">
		<div class="cms_c_inst neirong cf">
			<div class="left cf">
				...
				<i>></i> <a href="<%=adminPath%>contentExtendFiledController/list.do">属性管理</a> <i>></i> <i>修改属性</i>
			</div>
		</div>
			...
			<form id="Form" method="post">
				...
				<div class="all_btn cf">
					<span id="result"></span> <input type="button" class="submit_btn"
						...
						href="<%=adminPath%>/contentExtendFiledController/list.do" class="submit_re_btn">取消</a>
				</div>
