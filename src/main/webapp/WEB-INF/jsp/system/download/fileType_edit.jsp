	...
	$(function(){
		...
		$(document).on('mouseleave','.yulan',function(){
			...
			$("#download_name").blur(function(){
				$.ajax({
					type: "POST",
					url:adminPath+"downloadType/findcount.do",
					data:{download_name:$("#download_name").val(),id:$("#download_id").val()},
...
function setTypediv(param){
	...
	$.ajax({
		type: "GET",
		url:adminPath+"downloadType/getTree.do",
		data:{"DOWNLOAD_ID":param,"columId":columId},
		dataType:'json',
		cache: false,
		success: function(result){
			if(result.code==200&&result.data!=null){
...
function appendTypediv(list){
	 ...
	 $.each(list,function(index,obj){
		 var table= '';
		 html+='<li>';
		 var is_checked='<p><em class="show_btn"></em><label for="cc_'+obj.download_id+'"><input type="radio" att="'+obj.download_name+'" name="pid" value="'+obj.download_id+'" id="cc_'+obj.download_id+'" /><span>'+obj.download_name+'</span><i></i></label></p>';
		 $("#typediv input[type='radio']:checked").each(function(){
			if(this.value==obj.download_id){
				is_checked='<p><em class="show_btn"></em><label for="cc_'+obj.download_id+'"><input checked="checked" type="radio" att="'+obj.download_name+'" name="pid" value="'+obj.download_id+'" id="cc_'+obj.download_id+'" /><span>'+obj.download_name+'</span><i class="active"></i></label></p>';
			} 
...
function eachList(html,list){
	 ...
	 $.each(list,function(index,obj){
		 ...
		 $("#typediv input[type='radio']:checked").each(function(){
			if(this.value==obj.download_id){
				is_checked='<p><em class="show_btn"></em><label for="cc_'+obj.download_id+'"><input type="radio" checked="checked" att="'+obj.download_name+'" name="pid" value="'+obj.download_id+'" id="cc_'+obj.download_id+'" /><span>'+obj.download_name+'</span><i class="active"></i></label></p>';
			} 
		 });
		 html+=is_checked;
		 	if(obj.childList!=null && obj.childList.length>0){
...
if($ids.length==0){
	window.top.mesageTip("warn","请选择一个栏目");
	return false;
  } 
	var html="";
	$.each($ids,function(index,obj){
...
/* function sub(type){
	$("#TXT").val(getContent());
		...
		$.ajax({
			type: "POST",
			url:adminPath+"downloadType/edit.do",
			data:formData,
			dataType:'json',
			cache: false,
	        processData: false,
			...
			success: function(result){
				 if(result.success){
					 if(type=='1'){//继续添加
						 ...
						 location.href=adminPath+'downloadType/toAdd.do';
					 }else{
						 window.top.mesageTip("success","修改成功");
						 location.href=adminPath+'downloadType/list.do';
					 }
				 }else{
		...
		<div class="left cf">
			...
			<i>></i> <i>修改分类</i>
				...
				<div class="cms_c_list cf">
					...
					<form  id="Form" action="<%=adminPath %>downloadType/edit.do" method="post" enctype="multipart/form-data" >
						...
						<input type="hidden" id="download_id" name="download_id" value="${fileType.download_id }"/>
							...
							<dd><div class="dd_con">
								<div class="show_list" style="margin-top:0px;">
									<table class="ev">
										...
										<tbody id="typeBody">
											<c:if test="${!empty fileType.pid }">
												<tr>
													<td width='50%' >${fileType.pname }</td>
													<td>
														<a href='javascript:;' onclick='removeBody(this,"#typediv :checked")'>移除</a>
														<input type='hidden'  value="${fileType.pid }" />
													</td>
												</tr>
											</c:if>
							...
							<dd><div class="dd_con">
								<div class="show_list" style="margin-top:0px;">
									<table class="ev">
										...
										<tbody id="columnBody">
											<c:forEach items="${fileType.columConfigList }" var="v" varStatus="n">
												<tr><td width='50%' >${v.columName }</td><td><a href='javascript:;' onclick='removeBody(this,"#columndiv :checked")'>移除</a><input type='hidden'  value="${v.id }" /></td></tr>
											</c:forEach>
										</tbody>
									</table>
								</div>
						...
						<div class="zhaopin zp_con01 cf">
							<dl class="cf zp_dl">
								...
								<dd><div class="dd_con"><input type="text" id="download_name" name="download_name" value="${fileType.download_name }" required><span id="type_nameSpan" class="warn_tip"></span></div></dd>
								<dd class="zp_dd">
									<c:if test="${fileType.wburl != '' && fileType.wburl != null}">
										<label for="wlink"><input id="wlink" type="checkbox"><span>转向外部链接</span><i class="active"></i></label>
										<div class="dd_con" style="display: block;">
											<input type="text" style="width: 300px;" id="type_wburl" name="wburl" value="${fileType.wburl }" class="url">
										</div>
									</c:if>
									<c:if test="${fileType.wburl == '' || fileType.wburl == null}">
						...
						<dl class="cf">
							...
							<dd>
								<div class="dd_con">
									<input type="text" name="typeUrlName" id="typeUrlName" value="${fileType.typeUrlName }"/>
								</div>
							</dd>
						</dl>
							...
							<dd>
								...
								<ul>
									<li class="file_upload file_upload_img"><label for="file_name_img" style="float:left;">
										<input type="file" id="file_name_img" onchange="javascript:setImagePreview();" name="file"><em>上传图片</em></label>
										<c:choose>
											<c:when test="${not empty fileType.imgurl}">
												<i style="float:left;">${fileType.title }</i>
												<a href="javascript:;" class="remove_file" style="float:left;">删除</a>
												<div class="hid_div" style="height:0px;overflow:hidden;">
													<input type="hidden" name="imageid" value="${fileType.imageid }"/>
												</div>
												<div class="yulan" style="float:left;position:relative;padding-left:10px;">预览
													<div class="pro_img" style="display:none; position:absolute;left:0px;top:20px;">
														<div class="pro_img_big"><img id="preview" alt="" src="<%=imgPath%>${fileType.imgurl}" width="150px" height="180px"></div>
													</div>
												</div>
											</c:when>
											<c:otherwise>
						...
						<dl class="cf">
							...
							<dd>
								<div class="dd_con show_hide">
									...
									<c:if test="${fileType.status!=1 }">
										<label for="hide" ><input  type="radio" id="hide" name="status" value="1" /><span>显示</span></label>
									    <label for="show" class="active"><input checked="checked" type="radio" id="show" name="status" value="0" /><span>隐藏</span></label>
									</c:if>
								</div>
							</dd>
						</dl>
					...
					<dl class="cf">
							...
							<dd>
								<div class="dd_con show_hide">
									<c:if test="${fileType.is_home==0 }">
										<label for="hide1" ><input  type="radio" id="hide1" name="is_home" value="1" /><span>是</span></label>
									<label for="show1" class="active"><input type="radio" checked="checked"  id="show1" name="is_home" value="0" /><span>否</span></label>
									</c:if>
									<c:if test="${fileType.is_home!=0 }">
										<label for="show1" class="active"><input  type="radio" checked="checked" id="show1" name="is_home" value="1" /><span>是</span></label>
									<label for="hide1" ><input type="radio"  id="hide1" name="is_home" value="0" /><span>否</span></label>
									</c:if>
								</div>
							</dd>
						</dl>
								...
								<div class="dd_con" style="z-index:1;">
									<div class="eidt_box cf" style="z-index:1;">
										<table id="table_report" class="table table-striped table-bordered table-hover" style="z-index:1;">
												<tr>
													<td style="z-index:1;">
													  <script id="editor" type="text/plain" style="width:90.2%;height:240px;z-index:1;">${fileType.TXT }</script>
													</td>
												</tr>
							...
							<dd>
								<div class="dd_con">
									<ul>
										<li>
											<h6>
												<textarea class="textarea_numb" name="summary">${fileType.summary }</textarea>
							...
							<h3><span>选择分类</span><p class="close" onclick="popupHiden(this,'#typediv input','#typeBody input')">x</p></h3>
								<div class="layer_list_other mScrol222 cf" >
									<ul id="typediv" class="radio_lock">
										<input type="radio" checked="checked" name="pid" value="${fileType.pid }"></input>
					...
					<div class="layer_bg layer_bg02" style="display:none;">
						<div class="layer_con cf">
							...
							<div class="layer_list_other mScrol222 cf" >
								<ul id="columndiv">
								<c:forEach items="${fileType.columConfigList }" var="v" varStatus="num">
									<input type="checkbox" checked="checked" name="columnids" value="${v.id }"></input>
								</c:forEach>
						...
						<div class="layer_con cf">
							...
							<div class="layer_list_other mScrol222 cf" >
								<ul id="templatediv">
								<%-- <input type="radio" checked="checked"  name="templateid" value="${fileType.templateid }"/> --%>
									<div style="width:100%;text-align:center;height:50px;line-height:50px;font-size:16px;margin-top:50px;">无可选择的模板，请添加模板后，再操作</div>
								</ul>
							</div>
							<div class="all_btn cf">
									<input type="button" onclick="modalHide('#templatediv :checked','#templateBody','.layer_bg03');" class="submit_btn" value="确定" />
