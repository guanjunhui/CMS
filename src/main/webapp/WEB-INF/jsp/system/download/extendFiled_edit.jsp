...
<script type="text/javascript">
	$(function(){
		...
		$("#bname").blur(function(){
			$.ajax({
				type: "POST",
				url:adminPath+"downloadExtendFiledController/findCount.do",
...
function save(type){
	...
	var formData=$("#Form").jsonObject();
		$.ajax({
			type: "POST",
			url:adminPath+"downloadExtendFiledController/update.do",
			data:formData,
			success: function(result){
				 if(result.success){
					 if(type=='1'){//继续添加
						 window.top.mesageTip("success","操作成功");
						 location.href=adminPath+'downloadExtendFiledController/toAdd.do';
					 }else{
						 window.top.mesageTip("success","操作成功");
						 location.href=adminPath+'downloadExtendFiledController/list.do';
					 }
				 }else{
	...
	<div class="cms_con cf">
		<div class="cms_c_inst neirong cf">
			<div class="left cf">
				...
				<i>></i> <a href="<%=adminPath%>downloadExtendFiledController/list.do">属性管理</a> <i>></i> <i>修改属性</i>
			</div>
		</div>
			...
			<form id="Form" method="post">
				...
				<div class="all_btn cf">
					<span id="result"></span> <input type="button" class="submit_btn"
						...
						href="<%=adminPath%>/downloadExtendFiledController/list.do" class="submit_re_btn">取消</a>
				</div>
