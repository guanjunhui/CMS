...
<script type="text/javascript">
	$(function(){
		...
		$("#ss").click(function(){
			/* var $key=$("#key").val();
			$key=$.trim($key);
			window.location.href=adminPath+"userInfo/list.do?text="+encodeURI(encodeURI($key));  */
			$("#Form").submit();
		});
	});
	//当批量删除问题的时候提示是否删除
	...
	function deleteChecks(){
		...
		window.location.href=adminPath+"userInfo/deleteByIds.do?id="+ids.toString();		
	}
	
	//通过id删除数据
	function deleteById(data){
		window.location.href=adminPath+"userInfo/deleteById.do?id="+data;
	}
	//当删除问题的时候提示是否删除
	...
	function getReplyByTopicId(data){
		window.location.href=adminPath+"userInfo/replyList.do?queryKey="+data;
	}
	//根据id查看详情
	function detailById(id){
		window.location.href=adminPath+"userInfo/detailById.do?id="+id;
	}
</script>
<style>
.tit4{cursor:pointer;}
			...
			<div class="cms_con cf">
				...
				<div class="cms_c_inst neirong cf">
					<div class="left cf">
						...
						<i>></i>
						<a href="<%=adminPath%>userInfo/list.do">表单管理</a>
						<i>></i>
						<i>用户信息</i>
					</div>
				</div>
				<form id="Form" action="<%=adminPath%>userInfo/list.do" method="post">
				<div class="cms_c_list biaoge_con cf">
					<div class="h3 cf">
						<%-- <div class="h3_left cf"><a href="<%=adminPath%>problem/goAdd.do">+添加问题</a></div>	 --%>
						<div class="h3_left cf">
							<div class="a_btn cf">
								<%-- <a href="<%=adminPath%>userInfo/export.do"><img src="static/images/chu.png" alt="">导出</a> --%>
								<a href="<%=adminPath%>userInfo/download.do"><img src="static/images/chu.png" alt="">导出</a>
							</div>
						</div>			
						<div class="search">
							<input placeholder="请输入提问内容" type="text" id="key" name="text"><input id="ss" type="button"  value="搜索" />
						</div>
					</div>
					<div class="table cf">

						<dl class="list_bg col_09 cf" id="did">
							<dt class="cf">
								<div class="tit9">
									<div class="tit9_con">
										邮箱
								...
								<div class="sele09">操作</div>
								<div class="sele09">时间</div>
								<div class="sele09">订阅</div>
								<div class="sele09">来源</div>
								<div class="sele09">昵称</div>
								<div class="sele09">名</div>
								<div class="sele09">姓</div>
								<div class="sele09 other_width">电话</div>
							</dt>
										...
										<div class="dd_tit cf">
											...
											<div class="sele09 sanJi">
												...
												<ul class="guanli_con cf">
													...
													<li><a onclick="getReplyByTopicId('${map.id }')" href="javascript:;">回答列表</a></li> --%>
											...
											<div class="sele09">
												<c:if test="${map.isSub==1}">是</c:if>
												<c:if test="${map.isSub==0}">否</c:if>
											</div>
											<div class="sele09">
												<c:if test="${map.source==1}">IPC</c:if>
												<c:if test="${map.source==2}">PMM</c:if>
												<c:if test="${map.source==3}">ATV</c:if>
											</div>
											<div class="sele09">
												${map.nickname}
											</div>
