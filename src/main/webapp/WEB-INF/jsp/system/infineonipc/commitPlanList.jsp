...
<script type="text/javascript">
	$(function(){
		...
		$("#ss").click(function(){
			/* var $key=$("#key").val();
			$key=$.trim($key);
			window.location.href=adminPath+"commitPlan/list.do?keywords="+encodeURI(encodeURI($key));  */
			$("#Form").submit();
		});
	});
	//当批量删除问题的时候提示是否删除
	...
	function deleteChecks(){
		...
		window.location.href=adminPath+"commitPlan/deleteByIds.do?id="+ids.toString();		
	}
	
	//通过id删除数据
	function deleteById(data){
		window.location.href=adminPath+"commitPlan/deleteById.do?id="+data;
	}
	//当删除问题的时候提示是否删除
	...
	function updateHandle(id,sta){
		$.ajax({
            url:adminPath+"commitPlan/updateHandle",
            type:"post",
            data:{
            	"id":id,
            	"handle":sta
            ...
            success:function(data){
           	 	  if (data.code == 200) {
           			window.location.href="${basePath}/manage/commitPlan/list.do";
		          }else{
		        	  alert("服务器错误");
		          }
            }
	...
	function detailById(id){
		window.location.href=adminPath+"commitPlan/detailById.do?id="+id;
	}
</script>
<style>
.tit4{cursor:pointer;}
			...
			<div class="cms_con cf">
				...
				<div class="cms_c_inst neirong cf">
					<div class="left cf">
						...
						<i>></i>
						<a href="<%=adminPath%>commitPlan/list.do">表单管理</a>
						<i>></i>
						<i>我有方案</i>
					</div>
				</div>
				<form id="Form" action="<%=adminPath%>commitPlan/list.do" method="post">
				<div class="cms_c_list biaoge_con cf">
					
					<div class="h3 cf">
						<div class="h3_left cf">
							<div class="a_btn cf">
								<%-- <a href="<%=adminPath%>ipcExcel/download.do?key=2"><img src="static/images/chu.png" alt="">获取导入模板</a> --%>
								...
								<a href="<%=adminPath%>ipcExcel/export.do?key=2"><img src="static/images/chu.png" alt="">导出</a>
							</div>
						</div>
										...
										<div class="dd_tit cf">
											<div class="tit5">
												<div class="tit5_con">
													...
													<c:if test="${map.handle==1 }"><i style="color:green;">[已处理]</i>${map.planMsg}</c:if>
													<c:if test="${map.handle!=1 }">${map.planMsg}</c:if>
												</div>
											</div>
