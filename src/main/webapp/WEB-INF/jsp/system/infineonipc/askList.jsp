...
<script type="text/javascript">
	$(function(){
		...
		$("#ss").click(function(){
			/* var $key=$("#key").val();
			$key=$.trim($key);
			window.location.href=adminPath+"ipcAsk/list.do?text="+encodeURI(encodeURI($key));  */
			$("#Form").submit();
		});
	});
	//当批量删除问题的时候提示是否删除
	...
	function deleteChecks(){
		...
		window.location.href=adminPath+"ipcAsk/deleteByIds.do?id="+ids.toString();		
	}
	
	//通过id删除数据
	function deleteById(data){
		window.location.href=adminPath+"ipcAsk/deleteById.do?id="+data;
	}
	//当删除问题的时候提示是否删除
	...
	function getReplyByTopicId(data){
		window.location.href=adminPath+"ipcAsk/replyList.do?queryKey="+data;
	}
	//根据id查看详情
	function detailById(id){
		window.location.href=adminPath+"ipcAsk/detailById.do?id="+id;
	}
</script>
<style>
.tit4{cursor:pointer;}
			...
			<div class="cms_con cf">
				...
				<div class="cms_c_inst neirong cf">
					<div class="left cf">
						...
						<i>></i>
						<a href="<%=adminPath%>ipcAsk/list.do">表单管理</a>
						<i>></i>
						<i>技术问答</i>
					</div>
				</div>
				<form id="Form" action="<%=adminPath%>ipcAsk/list.do" method="post">
				<div class="cms_c_list biaoge_con cf">
					...
					<div class="table cf">
						...
						<dl class="list_bg col_08 cf" id="did">
							<dt class="cf">
								<div class="tit8">
									<div class="tit8_con">
										问答标题
								...
								<div class="sele08">操作</div>
								<div class="sele08">时间</div>
								<div class="sele08">状态</div>
								<div class="sele08">收藏量</div>
								<div class="sele08">浏览量</div>
								<div class="sele08">回复量</div>
								<div class="sele08 other_width">发表人</div>
							</dt>
									...
									<dd class="cf">
										<div class="dd_tit cf">
											...
											<div class="sele08">
												...
												<%-- <c:if test="${map.isClose==1}">已解决</c:if> --%>
												<c:if test="${map.isClose==0}">待解决</c:if>
											</div>
											<div class="sele08">
												${map.collCount}
											</div>
