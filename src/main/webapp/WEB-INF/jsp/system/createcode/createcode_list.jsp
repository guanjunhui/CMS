...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="<%=adminPath%>createCode/list.do" method="post" name="Form" id="Form">
	...
	<script type="text/javascript">
		...
		function productCode(CREATECODE_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="代码生成器";
			 diag.URL = '<%=adminPath%>createCode/goProductCode.do?CREATECODE_ID='+CREATECODE_ID;
			 diag.Width = 800;
			 diag.Height = 500;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=adminPath%>createCode/delete.do?CREATECODE_ID="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type:"POST",
								url:'<%=adminPath%>createCode/deleteAll.do?tm='+new Date().getTime(),
						    	data:{DATA_IDS:str},
								dataType:'json',
