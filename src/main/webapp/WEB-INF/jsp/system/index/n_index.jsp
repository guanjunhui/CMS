				...
				<ul class="nav01">
					...
					<c:forEach items="${menuList}" var="menu1" varStatus="stat">
						<c:if test="${menu1.hasMenu && '1' == menu1.MENU_STATE}">
							<li><a href="javascript:;" data-src="<%=adminPath%>left_nav/${menu1.MENU_ID }.do"><p>${menu1.MENU_NAME }</p></a></li>
						</c:if>
					</c:forEach>
				</ul>
			</div>
...
<div class="body_contact cf">
	<iframe id="leftiframe" src="<%=adminPath%>left_nav/${menuID }.do"></iframe>
</div>
...
var src=$("#leftiframe").attr("src");
$('.header_mid .nav01 a').removeClass('active');
$('.header_mid .nav01 li').each(function(index,item){
	var href=$(this).find('a').data('src');
	if(src==href){
		$(this).find('a').addClass('active');
	}
});

$(document).on('click','.header_mid .nav01 a',function(e){
	var Asrc=$(this).data('src');
	$('.header_mid .nav01 a').removeClass('active');
	$(this).addClass('active');
	$('.body_contact iframe').attr('src',Asrc);
});
