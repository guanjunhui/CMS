...
<body>
			...
			<div class="cms_con cf">
				...
				<div class="cms_c_list biaoge_con cf">
					<div class="h3 cf">
						<div class="search">
							<input placeholder="请输入提问内容" name="title" type="text" id="key"><input id="ss" type="button"  value="搜索" />
						</div>
					</div>
					<input type="hidden" name="columnId" value="0ac6d48dc93b42a499ef8ab9732e78b8">
					<div class="table cf">

						<dl class="list_bg col_03 cf" id="did">
							<dt class="cf">
								<div class="sele03 "></div>
								<div class="sele03">方案标题</div>
								<div class="sele03">问答数量</div>
								<div class="sele03">创建时间</div>
								<div class="sele03">操作</div>
							</dt>
							<c:if test="${!empty contentList}">
								<c:forEach items="${contentList}" var="l" varStatus="mun">
									<dd class="cf">
									<div class="dd_tit cf other_bg">
										<div class="sele03">${mun.count }</div>
										<div class="sele03">${l.title}</div>
										<div class="sele03">${l.num}</div>
										<div class="sele03 ">${l.createTime}</div>
										<div class="sele03">
											<input type="button" data-id="${l.id }" class="submit_btn right" style="margin-left:25%" value="查看问答" aria-invalid="false">
										</div>
									</div>
									</dd>
								</c:forEach>
