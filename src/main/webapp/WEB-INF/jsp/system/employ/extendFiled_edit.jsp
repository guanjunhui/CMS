...
<script type="text/javascript">
	$(function(){
		...
		$("#bname").blur(function(){
			$.ajax({
				type: "POST",
				url:adminPath+"employExtendFiledController/findCount.do",
...
function save(type){
	...
	var formData=$("#Form").jsonObject();
		$.ajax({
			type: "POST",
			url:adminPath+"employExtendFiledController/update.do",
			data:formData,
			success: function(result){
				 if(result.success){
					 if(type=='1'){//继续添加
						 window.top.mesageTip("success","操作成功");
						 location.href=adminPath+'employExtendFiledController/toAdd.do';
					 }else{
						 window.top.mesageTip("success","操作成功");
	...
	<div class="cms_con cf">
		<div class="cms_c_inst neirong cf">
			<div class="left cf">
				...
				<i>></i> <a href="<%=adminPath%>employExtendFiledController/list.do">招聘属性管理</a> <i>></i> <i>添加属性</i>
			</div>
		</div>
