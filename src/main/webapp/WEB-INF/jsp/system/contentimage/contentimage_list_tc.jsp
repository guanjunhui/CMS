...
<body class="no-skin">
	<div class="main-container" id="main-container">
		...
		<div class="main-content">
			<div class="main-content-inner">
					<!-- 检索  -->
					<form action="<%=adminPath%>contentImage/${msg}" method="post" name="Form" id="Form">
					<!-- <input name="xzvalue" id="xzvalue" value="" type="hidden" /> -->
					<%-- <input name="MASTER_ID" id="MASTER_ID" value="${pd.MASTER_ID}" type="hidden" /> --%>
							...
							<c:when test="${not empty varList}">
								...
								<c:forEach items="${varList}" var="var" varStatus="vs">
									<tr>
										...
										<td class="center">
										<a href="${pd.imageipport}<%=path%>/uploadFiles/uploadImgs/${var.contentImageUrl}" title="${var.contentImageTitle}" class="bwGal"><img src="${pd.imageipport}<%=path%>/uploadFiles/uploadImgs/${var.contentImageUrl}" alt="${var.contentImageTitle}" width="100"></a>
										</td>
										<td class="center">${var.contentImageTitle}</td>
										<td class="center" style="width:130px;">
											<%-- <a class="btn btn-xs btn-info" title="选择" onclick="xuanTp('<%=adminPath%>uploadFiles/uploadImgs/${var.PATH}');">选择 </a> --%>
											<c:if test="${QX.del == 1 }">
											<a style="cursor:pointer;" class="red" onclick="del('${var.contentImageId}','${var.contentImageUrl}');" title="删除">
	...
	<script type="text/javascript">
		...
		//删除
		function del(id,imgurl){
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=adminPath%>contentImage/delete.do?imageId="+id+"&imgurl="+imgurl+"&tm="+new Date().getTime();
