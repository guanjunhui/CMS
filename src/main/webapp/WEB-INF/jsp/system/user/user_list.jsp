			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="<%=adminPath%>user/listUsers.do" method="post" name="userForm" id="userForm">
...
function delUser(userId,msg){
	bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "<%=adminPath%>user/deleteU.do?USER_ID="+userId+"&tm="+new Date().getTime();
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=adminPath%>user/goAddU.do';
	 diag.Width = 469;
	 diag.Height = 510;
...
function editUser(user_id){
	 ...
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = '<%=adminPath%>user/goEditU.do?USER_ID='+user_id;
	 diag.Width = 469;
	 diag.Height = 510;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		if(result) {
			...
			if(str==''){
				...
				if(msg == '确定要删除选中的数据吗?'){
					...
					$.ajax({
						type: "POST",
						url: '<%=adminPath%>user/deleteAllU.do?tm='+new Date().getTime(),
				    	data: {USER_IDS:str},
						dataType:'json',
...
function sendSms(phone){
	 ...
	 diag.Drag=true;
	 diag.Title ="发送短信";
	 diag.URL = '<%=adminPath%>head/goSendSms.do?PHONE='+phone+'&msg=appuser';
	 diag.Width = 600;
	 diag.Height = 265;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function sendEmail(EMAIL){
	 ...
	 diag.Drag=true;
	 diag.Title ="发送电子邮件";
	 diag.URL = '<%=adminPath%>head/goSendEmail.do?EMAIL='+EMAIL;
	 diag.Width = 660;
	 diag.Height = 486;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function sendFhsms(username){
	 ...
	 diag.Drag=true;
	 diag.Title ="站内信";
	 diag.URL = '<%=adminPath%>fhsms/goAdd.do?username='+username;
	 diag.Width = 660;
	 diag.Height = 444;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function toExcel(){
	...
	window.location.href='<%=adminPath%>user/excel.do?keywords='+keywords+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&ROLE_ID='+ROLE_ID;
}
...
function fromExcel(){
	 ...
	 diag.URL = '<%=adminPath%>user/goUploadExcel.do';
	 diag.Width = 300;
	 diag.Height = 150;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
function viewUser(USERNAME){
	if('admin' == USERNAME){
	 ...
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = '<%=adminPath%>user/view.do?USERNAME='+USERNAME;
	 diag.Width = 469;
	 diag.Height = 380;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
