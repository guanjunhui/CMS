...
function columnHide(param){
	$("#columndiv").find("input").removeAttr("checked");
	$("#columndiv li babel").removeClass('radio_btn');
	$("#columndiv").find('i').removeClass('active');
	$("#columnSpan").html("");
  	$(param).parents('.layer_bg').hide();
}
function columnClose(param){
	$("#columndiv").find("input").removeAttr("checked");
	$("#columndiv li babel").removeClass('radio_btn');
	$("#columndiv").find('i').removeClass('active');
	$("#columnSpan").html("");
...
function modalHide1A(){//栏目窗口取消
	$("#columndiv input[type='checkbox']:checked").attr("checked",false);
	$('.layer_bg08').hide();
}
/* function modalHide(){
	var $ids=$("#typediv input[type='radio']:checked");
	 if($ids.length==0){
		 $('.layer_bg08').hide();
		return false;
	   } 
...
function modalHideA(){
		$("#typediv input[type='radio']:checked").attr("checked",false);
		$('.layer_bg08').hide();
}
//保存类型
function save(type){
	...
	if(($('#cmslwt').css('display'))!='none'){
		window.top.mesageTip("warn","该分类名称已经存在,请重新输入!");
		return false;
	} 
...
/* function sub(type){
		...
		$.ajax({
			type: "POST",
			url:adminPath+"mymessageType/save.do",
			data:formData,
			dataType:'json',
			cache: false,
	        processData: false,
...
//验证类型名是否重复
function onlyTypeName(family){
	var typename=$("#type_name").val();
	$.ajax({
			type: "GET",
			url:adminPath+"mymessageType/judgeTypeName.do",
			data:"typeName="+encodeURI(encodeURI(typename)),
			dataType:'json',
			cache: false,
	        processData: false,
	        contentType: false,
	...
	<!-- cms_con开始 -->
			<div class="cms_con cf">
				...
				<div class="cms_c_list cf">
					...
					<form  name="Form" action="<%=adminPath%>mymessageType/save.do" id="Form" method="post" enctype="multipart/form-data" >
						...
						<div class="zhaopin zp_con01 cf">
							<dl class="cf zp_dl">
								...
								<dd><div class="dd_con" style="position:relative;"><input type="text" onblur="onlyTypeName(this)" id="type_name" name="type_name" >
								<span id="cmslwt" style="color:red;display:none; position:absolute;right:75px;top:8px;;">分类名称重复,请重新输入!</span>
								</div></dd>
								<dd class="zp_dd">
									<label for="wlink"><input id="wlink" type="checkbox"><span>转向外部链接</span><i></i></label>
									<div class="dd_con"><input type="text" style="width:300px;" id="type_wburl" name="type_wburl" ></div>
								</dd>
							</dl>
						</div>
						...
						<dl class="cf">
							<dt>关键字</dt>
							<dd><div class="dd_con"><input type="text" name="type_keywords" /></div></dd>
						</dl>
					...
					<span id="result"></span>
						...
						<a href="<%=adminPath%>/mymessageType/list.do" class="submit_re_btn">取消</a>
					</div>
					<!-- 分类弹窗——相关内容 -->
