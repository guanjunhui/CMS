	...
	<script type="text/javascript">
		...
		function parentReload(){
			if(null != ${MSG} && 'change' == ${MSG}){
				parent.location.href="<%=adminPath%>menu/listAllMenu.do?MENU_ID="+${MENU_ID};
			}
		}
		parentReload();
	</script>
...
<body class="no-skin">
	...
	<script type="text/javascript">
		...
		function goSonmenu(MENU_ID){
			top.jzts();
			window.location.href="<%=adminPath%>menu.do?MENU_ID="+MENU_ID;
		};
		
		//新增菜单
		function addmenu(MENU_ID){
			top.jzts();
			window.location.href="<%=adminPath%>menu/toAdd.do?MENU_ID="+MENU_ID;
		};
		
		//编辑菜单
		function editmenu(MENU_ID){
			top.jzts();
			window.location.href="<%=adminPath%>menu/toEdit.do?id="+MENU_ID;
		};
		...
		function delmenu(MENU_ID){
			bootbox.confirm("确定要删除此菜单吗?", function(result) {
				if(result) {
					var url = "<%=adminPath%>menu/delete.do?MENU_ID="+MENU_ID+"&guid="+new Date().getTime();
					top.jzts();
		...
		function editTb(MENU_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑图标";
			 diag.URL = '<%=adminPath%>menu/toEditicon.do?MENU_ID='+MENU_ID;
			 diag.Width = 800;
			 diag.Height = 380;
