...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="<%=adminPath%>codeeditor/list.do" method="post" name="Form" id="Form">
	...
	<script type="text/javascript">
		...
		function reduction(CODEEDITOR_ID){
			...
			$.ajax({
				type: "POST",
				url: '<%=adminPath%>codeeditor/reduction.do',
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=adminPath%>codeeditor/delete.do?CODEEDITOR_ID="+Id+"&tm="+new Date().getTime();
		...
		function view(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="历史编辑";
			 diag.URL = '<%=adminPath%>codeeditor/view.do?CODEEDITOR_ID='+Id;
			 diag.Width = 900;
			 diag.Height = 600;
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=adminPath%>codeeditor/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
