			...
			if(null != strIWATERM && !"".equals(strIWATERM)){
				...
				if(strIW.length == 4){
					if("yes".equals(strIW[0])){
						pressImage(SystemConfig.getPropertiesString("filepath.img", PathUtil.getUploadPath() + Const.FILEPATHIMG)+strIW[1], imagePath, Integer.parseInt(strIW[2]), Integer.parseInt(strIW[3]));
					}
				}
			}
		  } 
