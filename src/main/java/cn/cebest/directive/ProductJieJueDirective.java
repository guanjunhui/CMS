 ...
 * 栏目详细标签
 */
@Component("product_jiejuefangan")
public class ProductJieJueDirective implements TemplateDirectiveModel {
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars,
		...
		List<Brand> product=null;
		try {
			//根据产品Id和获取产品详情
			product = productService.findProductJieJueById(productId);
		} catch (Exception e) {
			logger.error("find the colum by ID ["+productId+"]ocurred error!",e);
		}
		//设置输出变量
        env.setVariable(Const.OUT_LIST, getBeansWrapper().wrap(product));
        if (body != null) {
        	//输出变量
