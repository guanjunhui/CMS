 ...
 * 栏目详细标签
 */
@Component("colum_detail")
public class ColumDetailDirective implements TemplateDirectiveModel {
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars,
		...
		PageData data=null;
		try {
			data = columconfigService.findColumconfigById(pd);
		} catch (Exception e) {
			logger.error("find the colum by ID ["+columId+"]ocurred error!",e);
		}
		//设置输出变量
        env.setVariable(Const.OUT_BEAN, getBeansWrapper().wrap(data));
        if (body != null) {
        	//输出变量
